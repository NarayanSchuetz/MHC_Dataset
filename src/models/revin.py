...
# RevIN Class (from https://github.com/ts-kim/RevIN)
class RevIN(nn.Module):
    ...
    def forward(self, x, mode:str):
        ...
        else: raise NotImplementedError(f"Unknown mode: {mode}")
        return x
    ...
    def _denormalize(self, x):
        if self.affine:
            x = x - self.affine_bias
            x = x / (self.affine_weight + self.eps*self.eps) # Adding eps for stability
        x = x * self.stdev
        if self.subtract_last:
            x = x + self.last
        else:
